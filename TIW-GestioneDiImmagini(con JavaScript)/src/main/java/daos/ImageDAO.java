...
public class ImageDAO implements MyDAO
	{
		...
		public List<Image> getAtomicUserImagesInfos (String username) throws DbErrorException, BadFormatException
			{
				...
				try
					{
						...
						userImagesInfos = this.getUserImagesInfos(username);
			
						this.connection.commit();
				
						return (userImagesInfos);
					}
		...
		public List<Image> getUserImagesInfos (String username) throws DbErrorException, BadFormatException
			{
				String query = "SELECT titolo FROM " + MyDAO.IMAGES_TABLE_NAME + " WHERE creatore=?;";

				ResultSet result_query = null;
				PreparedStatement pstatement_query = null;
				List<Image> ret = null;

				try
					{
						User.checkUsername(username);
						pstatement_query = connection.prepareStatement(query);
						pstatement_query.setString(1, username);
						result_query = pstatement_query.executeQuery();
						ret = new ArrayList<>();
						
						while (result_query.next())
							{
								ret.add(this.getImage(username, result_query.getString("titolo"), username));
							}
					...
					{
						MyDAO.closeQuery(result_query, pstatement_query);
					}
			}
		...
		public Image getAtomicImage (String creator, String image_name, String requester) throws DbErrorException, BadFormatException
			{
				Image image;
				...
				try
					{
						...
						image = this.getImage(creator, image_name, requester);
